%\VignetteIndexEntry{Bar, Surface and Other Plots}
...
\stitle {barsurf}{0.2.0}{Bar, Surface and Other Plots}
...
Currently, it contains six main functions:
\begin {itemize}
	\item plot2d.cell()
	\item plot2d.contour()
...
Note that the x and y arguments are ignored, if supplied.
