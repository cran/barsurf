...
\stitle {barsurf}{0.5.0}{Heatmap-Related Plots\\and\\Smooth\\Multiband Color Interpolation}
...
Also, the misc3d package needs to be installed and on the search path, in order to use the functions for 3D contour plots.
...
We could use a larger n value, or (more efficiently) superimpose a smaller heatmap on top of a larger heatmap, in the area of high curvature. Either way, any improvement is mainly aesthetic.
